...
snappy_status snappy_decompress_cuda(struct host_buffer_context *input, struct host_buffer_context *output, struct program_runtime *runtime)
{
	...
	{
		if (total_blocks >= 1024 && total_blocks <= 4096)
		{
			block.x = total_blocks/512;
			grid.x = (unsigned int) ceil(total_blocks * 1.0 / block.x);
		}
		if (total_blocks > 4096 && total_blocks <= 8192)
		{
			block.x = total_blocks/128;
			grid.x = (unsigned int) ceil(total_blocks * 1.0 / block.x);
		}
		if (total_blocks > 8192 && total_blocks < 65536)
		{
			block.x = total_blocks/2048;
			grid.x = (unsigned int) ceil(total_blocks * 1.0 / block.x);
		}
		if (total_blocks >= 65536)
		{
			block.x = total_blocks/8192;
			grid.x = (unsigned int) ceil(total_blocks * 1.0 / block.x);
		}
	}
	else
	...
	struct timeval start;
	gettimeofday(&start, NULL);
	fprintf(stderr, ":before GPU:%.4f", start.tv_sec + start.tv_usec / 1000000.0);
	snappy_decompress_kernel<<<grid,block,0>>>(input, output, total_blocks, dblock_size, input_offsets, input_currents);
	gettimeofday(&start, NULL);
	fprintf(stderr, ":after GPU:%.4f", start.tv_sec + start.tv_usec / 1000000.0);
