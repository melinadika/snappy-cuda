...
/*
 * Allocate the decompression memory buffers that will be reused across
 * multiple calls to the decompressor.
 *
...
/*
 * Initialize decompressor without reallocating memory buffers.
 *
 * The caller is expected to provide a large enough buffer to hold the output
 * data. No safety checks are made.
 *
...
void decompressor_init(
    ...
    void *gpu_output_buf, size_t gpu_output_size, void *reader_ctx);
